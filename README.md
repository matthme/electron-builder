> [!NOTE]
> **This is a fork of electron-builder that modifies the electron-updater in a way that makes it look specifically for new semver-compatible releases**. Without this change, electron-updater would just fetch the latest release on github, no matter whether it is semver-compatible or not, which prevents older semver verison lines to receive updates, if incompatible releases are rejected in app code.
> See https://github.com/holochain/launcher/issues/125
